...
do
  ...
  job=$(sbatch --parsable --job-name="CImvg-${CI_COMMIT_SHORT_SHA}" ${slurm_opts} --nodes=${nodes} --wrap="source $inst-Release/env.sh ; module list ; env|grep SLURM; env|grep GAS; env|grep UPC; env|grep FI; set -x; ${RWDI} $OPTS -o ${RUN_PREFIX}/$arch-rwdi-0 && echo 'sleeping for 15s so that srun can recover' && sleep 15 && ${DBG} ${OPTS} --kmer-lens 63 --scaff-kmer-lens 63 --use-qf=0 -o ${RUN_PREFIX}/$arch-dbg-0-k63 && echo Good")
...
do
  ...
  cmds="$cmds $inst-Release/bin/check_asm_quality.py --asm-dir ${RUN_PREFIX}/${d1} --expected-quals ${inst}-Release/share/good-arctic-sample0.txt --refs ${ARCTIC}/arcticsynth-refs.fa & $inst-Release/bin/check_asm_quality.py --asm-dir ${RUN_PREFIX}/${d2} --expected-quals ${inst}-Release/share/good-arctic-sample0-k63.txt --refs ${ARCTIC}/arcticsynth-refs.fa &"
  waits="$waits && wait -n && wait -n "
done
...
sbatch --output=$OUT --wait --qos=debug --time=30:00 --account=m2865 -C cpu -c 16 --wrap="$cmds $waits"
cat $OUT
