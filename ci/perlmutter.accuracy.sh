...
do
  ...
  job=$(sbatch --parsable ${slurm_opts} --nodes=$nodes --time=8:00 --wrap="set -x ; source $REL/env.sh; module list; env|grep SLURM; env|grep UPC; env|grep FI; set -x ; ${REL} $OPTS -o ${RUN_PREFIX}/$arch-rel && echo Good")
...
do
  ...
  cmds="$cmds ${REL}/bin/check_asm_quality.py --asm-dir ${RUN_PREFIX}/${arch}-rel --expected-quals ${REL}/share/good-arcticsynth.txt --refs ${ARCTIC}/arcticsynth-refs.fa &"
  waits="$waits && wait -n"
done
