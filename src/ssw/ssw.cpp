...
static const int8_t kBaseTranslation[128] = {4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
...
void BuildSwScoreMatrix(const uint8_t& match_score, const uint8_t& mismatch_penalty, const uint8_t& ambiguity_penalty,
                        int8_t* matrix) {
...
Aligner::Aligner(const uint8_t& match_score, const uint8_t& mismatch_penalty, const uint8_t& gap_opening_penalty,
                 const uint8_t& gap_extending_penalty, const uint8_t& ambiguity_penalty)
...
Aligner::Aligner(const int8_t* score_matrix, const int& score_matrix_size, const int8_t* translation_matrix,
...
bool Aligner::Align(const char* query, const Filter& filter, Alignment* alignment, const int32_t maskLen) const {
  ...
  s_align* s_al = ssw_align(profile, translated_reference_, reference_length_, static_cast<int>(gap_opening_penalty_),
                            static_cast<int>(gap_extending_penalty_), flag, filter.score_filter, filter.distance_filter, maskLen);
...
bool Aligner::Align(const char* query, const char* ref, const int& ref_len, const Filter& filter, Alignment* alignment,
                    const int32_t maskLen) const {
  return Align(query, strlen(query), ref, ref_len, filter, alignment, maskLen);
}

bool Aligner::Align(const char* query, const int& query_len, const char* ref, const int& ref_len, const Filter& filter,
  ...
  uint8_t flag = 0;
  SetFlag(filter, &flag);
  s_align* s_al = ssw_align(profile, translated_ref, valid_ref_len, static_cast<int>(gap_opening_penalty_),
                            static_cast<int>(gap_extending_penalty_), flag, filter.score_filter, filter.distance_filter, maskLen);
...
bool Aligner::ReBuild(const uint8_t& match_score, const uint8_t& mismatch_penalty, const uint8_t& gap_opening_penalty,
                      const uint8_t& gap_extending_penalty, const uint8_t& ambiguity_penalty) {
  if (translation_matrix_) return false;
...
bool Aligner::ReBuild(std::string scores) {
  if (translation_matrix_) return false;
  SetAllDefault();

  if (scores.length() != 5) return false;
  match_score_ = scores[0] - '0';
  mismatch_penalty_ = scores[1] - '0';
  gap_opening_penalty_ = scores[2] - '0';
  gap_extending_penalty_ = scores[3] - '0';
  ambiguity_penalty_ = scores[4] - '0';
  BuildDefaultMatrix();
  return true;
}

bool Aligner::ReBuild(const int8_t* score_matrix, const int& score_matrix_size, const int8_t* translation_matrix,
                      const int& translation_matrix_size) {
  ClearMatrices();
  score_matrix_ = new int8_t[score_matrix_size_ * score_matrix_size_];
  memcpy(score_matrix_, score_matrix, sizeof(int8_t) * score_matrix_size_ * score_matrix_size_);
