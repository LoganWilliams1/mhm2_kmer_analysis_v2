...
#define SKA_NOINLINE(...) __VA_ARGS__
#else
#ifdef _MSC_VER
...
struct prime_number_hash_policy {
  ...
  mod_function next_size_over(size_t &size) const {
    ...
    // 4. to fill the gaps for every prime p go to wolfram alpha and get ClosestPrime(p * 2^(1/3)) and ClosestPrime(p * 2^(2/3)) and
    // put those in the gaps
...
class flat_hash_set
  ...
  using Table =
      detailv3::sherwood_v3_table<T, T, H, detailv3::functor_storage<size_t, H>, E, detailv3::functor_storage<bool, E>, A,
                                  typename std::allocator_traits<A>::template rebind_alloc<detailv3::sherwood_v3_entry<T>>>;
